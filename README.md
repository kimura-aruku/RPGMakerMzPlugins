...
### 注意事項
- 本リポジトリで「あるく」が作成・改変したプログラム部分は、[MIT License](LICENSE)のもとで公開されています。
...
- 派生元プラグインおよび依存プラグインには、それぞれの著作権と利用条件が適用されます。各配布元の規約を確認してください。
...
本リポジトリで「あるく」が作成・改変したプログラム部分は、MIT Licenseのもとで公開されています。

- Copyright (c) 2025-2026 あるく
- ライセンス全文: [LICENSE](LICENSE)

`ARTM_TMDescriptionExMZExByAlk.js` に含まれる派生元のコードには、ファイル内に記載された原作者の著作権表示とMIT Licenseが引き続き適用されます。これらの表示は削除しないでください。

`ZZ_PluginCombiner.js` が連携する依存プラグインは本リポジトリに含まれません。依存プラグインの著作権および利用条件は、それぞれの配布元が定める規約に従います。

## スクリーンショットについて

掲載画像は、正規購入したRPGツクールMZを使用し、プラグインの動作説明を目的として撮影したものです。画像内に表示されるRPGツクールMZの標準素材その他の第三者著作物は、本リポジトリのMIT Licenseの対象には含まれません。各素材の著作権は、それぞれの権利者に帰属します。
