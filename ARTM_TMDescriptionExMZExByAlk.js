// ===================================================
// プラグイン名: ARTM_TMDescriptionExMZExByAlk
 ...
 * @author あるく
 *
 * @param isEnableTraitSkillAddDescription
 * @desc 特徴「スキル追加」で紐づくスキルの詳細確認機能を有効化
 * @type boolean
 * @default true
 * 
 * @param isVisiblePageNumber
 * @desc ページ番号の表示を有効化
 * @type boolean
 * @default true
 * 
 * @param pageNumberPositionType
 * @desc ページ番号の表示位置（0:上部罫線の下、1:下部罫線の上、2:右下）
 * @type number
 * @default 1
 * 
 ...
 *
 * @help ARTM_TMDescriptionExMZExByAlk
 *
...
TMPlugin.DescriptionEx.Parameters = PluginManager.parameters("ARTM_TMDescriptionExMZExByAlk");
...
// 追加
TMPlugin.DescriptionEx.IsEnableTraitSkillAddDescription = 
    TMPlugin.DescriptionEx.Parameters["isEnableTraitSkillAddDescription"] === 'true';
TMPlugin.DescriptionEx.IsVisiblePageNumber = 
    TMPlugin.DescriptionEx.Parameters["isVisiblePageNumber"] === 'true';
TMPlugin.DescriptionEx.PageNumberPositionType = 
    Number(TMPlugin.DescriptionEx.Parameters["pageNumberPositionType"]);
...
(() => {
    ...
    Window_DescriptionEx.prototype.refresh = function() {
        ...
        // 上下の表示位置
        let y;
        switch(descEx.PageNumberPositionType){
            // 上部罫線の下
            case 0:
                y = this.lineHeight() + descEx.HorzLineHeight;
                break;
            // 下部罫線の上
            case 1:
                y = this.profileY() - this.lineHeight() - this.itemPadding() * 2 - descEx.HorzLineHeight;
                break;
            // 右下
            case 2:
                y = this.profileY() - this.itemPadding() * 2 + descEx.HorzLineHeight;
                break;
        }
        const maxWidth = this.contents.width - this.itemPadding();
        this.drawText(pageString, 0, y, maxWidth, 'right');
    };
    ...
    // ページ更新
    Scene_Base.prototype.updateDescriptionPage = function() {
        if(!this._descriptionSkillList || this._descriptionSkillList.length === 0){
            return;
        }
        // 最初のページは元々のアイテム
        this._descriptionExWindow.currentPageNumber = this._currentDescriptionPageIndex + 1;
        if(this._currentDescriptionPageIndex === 0){
            this._descriptionExWindow.setItem(this._originalItem);
        }else{
            const skill = this._descriptionSkillList[this._currentDescriptionPageIndex - 1];
            this._descriptionExWindow.setItem(skill);
        }
    };

    // 次のページ
    Scene_Base.prototype.updateDescriptionPagedown = function() {
        const maxIndex = this._descriptionSkillList.length;
        // 最後のページなら最初に戻る
        if (this._currentDescriptionPageIndex === maxIndex) {
            this._currentDescriptionPageIndex = 0;
        }else{
            this._currentDescriptionPageIndex++;
        }
        SoundManager.playCursor();
        this.updateDescriptionPage();
    };

    // 前のページ
    Scene_Base.prototype.updateDescriptionPageup = function() {
        // 最初のページなら最後に戻る
        if (this._currentDescriptionPageIndex === 0) {
            const maxIndex = this._descriptionSkillList.length;
            this._currentDescriptionPageIndex = maxIndex;
        }else{
            this._currentDescriptionPageIndex--;
        }
        SoundManager.playCursor();
    ...
    Scene_Base.prototype.update = function() {
        ...
        if ((Input.isRepeated('left') || Input.isRepeated('pageup')) && 
            (this._descriptionSkillList && this._descriptionSkillList.length > 0)) {
                this.updateDescriptionPageup();
        } else if ((Input.isRepeated('right') || Input.isRepeated('pagedown')) && 
            (this._descriptionSkillList && this._descriptionSkillList.length > 0)){
                this.updateDescriptionPagedown();
        }
    };
